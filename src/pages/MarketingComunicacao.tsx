...
const MarketingComunicacao = () => {
  ...
  const areaName = "Marketing e Comunicação";
  const areaDescription = "Conecte-se com especialistas em marketing digital, branding, comunicação corporativa e estratégias de crescimento.";
  ...
  const loadMentors = async () => {
    try {
      ...
      const filteredMentors = mentorData.filter(mentor => 
        ...
        mentor.setor?.toLowerCase().includes('branding') ||
        mentor.setor?.toLowerCase().includes('social media') ||
        mentor.setor?.toLowerCase().includes('redes sociais') ||
        mentor.setor?.toLowerCase().includes('conteúdo') ||
        mentor.setor?.toLowerCase().includes('conteudo') ||
            ...
            {allMentors.length > 0 ? (
              ...
              <div className="text-center py-20">
                <div className="bg-gradient-to-br from-impulso-light/10 to-impulso-dark/10 rounded-full w-32 h-32 flex items-center justify-center mx-auto mb-6">
                  <TrendingUp className="w-16 h-16 text-impulso-light" />
                </div>
...
export default MarketingComunicacao; 